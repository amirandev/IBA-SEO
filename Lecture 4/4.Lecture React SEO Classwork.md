...
## **სლაიდი 1: შესავალი**
### **React SEO — პრაქტიკული ქლასვორკი**

🚀 დღეს ჩვენ გადავალთ თეორიიდან პრაქტიკაში! ავაწყობთ React აპლიკაციას, რომელიც იქნება სრულად ოპტიმიზირებული საძიებო სისტემებისთვის.
...
### **1. Lighthouse (ჩაშენებული Chrome-ში)**
- **როგორ გავხსნათ:** F12 → Lighthouse tab
...
- **რისთვის:** React კომპონენტების დებაგი, State/Props-ის ნახვა, Performance profiling

### **3. SEO Meta in 1 Click**
- [SEO Meta in 1 Click](https://chrome.google.com/webstore/detail/seo-meta-in-1-click/biogpclljnldjgolhnlahmlkhbfnanco)
- **რისთვის:** სწრაფად აჩვენებს ყველა მეტა თეგს, OG თეგებს, სტრუქტურირებულ მონაცემებს
...
### **5. PageSpeed Insights (პირდაპირი ბმული)**
- უბრალოდ გახსენით: **https://pagespeed.web.dev/**
- ან გამოიყენეთ ჩაშენებული **Lighthouse** Chrome DevTools-ში

### **6. WhatFont / Wappalyzer**
- **Wappalyzer:** გაიგეთ რა ტექნოლოგიებზეა აწყობილი ნებისმიერი საიტი
- **WhatFont:** შეამოწმეთ საიტზე გამოყენებული ფონტები

### **📌 დავალება #1:**
> დააინსტალირეთ **React Developer Tools**, **SEO Meta in 1 Click** და **Open Graph Debugger** Chrome-ში. გახსენით ნებისმიერი საიტი (მაგ: `facebook.com`) — SEO Meta-ით ნახეთ მეტა თეგები, OG Debugger-ით ნახეთ OG თეგები.
...
### **რა მოხდა?**
- `npm create vite@latest` — შექმნა ახალი React პროექტი Vite build tool-ით
- `npm install` — ჩამოტვირთა ყველა დამოკიდებულება (dependencies)
- `npm run dev` — გაუშვა ლოკალური სერვერი პორტზე 5173
...
### **პროექტის Build და ჰოსტინგზე ატვირთვა**
...
npm run build
```
✅ შეიქმნება `dist/` საქაღალდე — ეს არის მზა, გამოსაქვეყნებელი ვებსაიტი.
...
## **სლაიდი 12: 11. Advanced — Structured Data JSON-LD**
### **Schema.org მონაცემების დამატება React-ში**
...
export default function Seo({ title, description, path }) {
  ...
  const jsonLd = {
    ...
    url: siteUrl,
    description: 'React SEO პრაქტიკული ქლასვორკი'
  };
  ...
  return (
    <Helmet>
      ...
      <meta property="og:title" content={`${title} | React SEO Project`} />
...
### **📌 დავალება #9 (Bonus):**
> 1. შექმენით `Seo.jsx` კომპონენტი
> 2. დაამატეთ JSON-LD სტრუქტურირებული მონაცემები
> 3. გამოიყენეთ Helmet-ის მეშვეობით script tag-ის ჩასმა
...
|---|-----------|------------|-------|
| 1 | **React Developer Tools** | React კომპონენტების დებაგი | [Chrome Web Store](https://chrome.google.com/webstore/detail/react-developer-tools/fmkadmapgofadopljbjfkapdkoienihi) |
| 2 | **Lighthouse** | Performance + SEO + Accessibility | F12 → Lighthouse tab |
| 3 | **SEO Meta in 1 Click** | მეტა თეგების სწრაფი შემოწმება | [Chrome Web Store](https://chrome.google.com/webstore/detail/seo-meta-in-1-click/biogpclljnldjgolhnlahmlkhbfnanco) |
| 4 | **Open Graph Debugger** | OG (og:) თეგების შემოწმება | [Chrome Web Store](https://chromewebstore.google.com/detail/open-graph-debugger/bepabeobdmmefhhhkonppcppgodhjdno) |
| 5 | **Wappalyzer** | საიტის ტექნოლოგიების იდენტიფიკაცია | [Chrome Web Store](https://www.wappalyzer.com/app/) |

### **📌 დავალება #10:**
> 1. დააინსტალირეთ **React Developer Tools** Chrome Extension
> 2. გახსენით Components tab — ნახეთ თქვენი React კომპონენტები
> 3. გახსენით Profiler tab — ჩაწერეთ Performance სესია
...
📁 src/components/Breadcrumbs.jsx  → BreadcrumbList JSON-LD schema
📁 src/pages/Home.jsx              → 1x <h1>, ყველა img-ს alt, ყველა Link-ს title
📁 src/pages/ProductDetail.jsx     → Product Schema JSON-LD (price, rating, brand)
📁 src/pages/About.jsx             → Organization Schema JSON-LD
📁 src/pages/Sitemap.jsx           → HTML Sitemap (ადამიანებისთვის)
...
❌ 16. No JSON-LD Schema         pages/        → Google-ს არ აქვს სტრუქტურირებული მონაცემები
...
| **Structured Data** | Google-ს აწვდის **სტრუქტურირებულ** ინფორმაციას (ფასი, რეიტინგი, მარაგი). Rich Snippets ზრდის CTR-ს 30%-მდე. | JSON-LD `<script type="application/ld+json">` |
...
6. Structured Data → Google-მა იცის ფასი, რეიტინგი, მარაგი
...
| 13 | `public/robots.txt` (`Disallow: /admin/`) + `public/sitemap.xml` | ☐ |
| 14 | **HTML Sitemap** — `/sitemap` გვერდი React-ში (ადამიანებისთვის + Google-ისთვის) | ☐ |
| 15 | `npm run build` + Deploy (Byet Host / Vercel) | ☐ |
| 16 | Google Search Console — Sitemap submission | ☐ |
| 17 | SEO Meta Check — ყველა გვერდზე | ☐ |
| 18 | Bonus: Seo.jsx კომპონენტი JSON-LD-ით | ☐ |
